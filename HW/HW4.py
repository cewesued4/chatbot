...
zip_path = "su_orgs.zip"
extract_path = "su_orgs"
...
#We are using adaptive chunking since each paragraph in the various html documents are different lengths. 
#Additionally, these documents display a combination of either multiple paragraphs or a single paragraph. Due to this variety,
#we need to be adaptive in our chunking method. 
def chunk_text(text):
    #Here, we can split a document into two roughly equal chunks, depending on document length.
    midpoint = len(text) // 2

    #split at a nearby paragraph break
...
#text = extracted text from HTML files
...
st.title("HW4: Chatbot using RAG and chunking")
